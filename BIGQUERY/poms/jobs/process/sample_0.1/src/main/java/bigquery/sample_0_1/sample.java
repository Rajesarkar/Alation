...
public class sample implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "sample.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(sample.class);
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(sample.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_BIGQUERY_sample = new byte[0];
		static byte[] commonByteArray_BIGQUERY_sample = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_BIGQUERY_sample.length) {
					if (length < 1024 && commonByteArray_BIGQUERY_sample.length == 0) {
						commonByteArray_BIGQUERY_sample = new byte[1024];
					} else {
						commonByteArray_BIGQUERY_sample = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_BIGQUERY_sample, 0, length);
				strReturn = new String(commonByteArray_BIGQUERY_sample, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_BIGQUERY_sample, 0, length);
				strReturn = new String(commonByteArray_BIGQUERY_sample, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_BIGQUERY_sample) {
	...
	public void tRowGenerator_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				util_tLogRow_1.addRow(new String[] { "City", });
...
///////////////////////		
					...
					if (row1.City != null) { //
						row_tLogRow_1[0] = String.valueOf(row1.City);
...
///////////////////////    			
	...
	public int runJobInTOS(String[] args) {
		...
		org.slf4j.MDC.put("_jobRepositoryId", "_6IqBkJT-Ee-43stZsMkBDA");
		org.slf4j.MDC.put("_compiledAtTimestamp", "2024-10-28T07:34:32.958127600Z");
		...
		try {
			...
			java.io.InputStream inContext = sample.class.getClassLoader()
					.getResourceAsStream("bigquery/sample_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = sample.class.getClassLoader()
		...
		log.info("TalendJob: 'sample' - Started.");
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : sample");
		}
		if (enableLogStash) {
		...
		log.info("TalendJob: 'sample' - Finished - status: " + status + " returnCode: " + returnCode);
...
/************************************************************************************************
 * 64929 characters generated by Talend Real-time Big Data Platform on the 28
 * October 2024 at 1:04:32 pm IST
 ************************************************************************************************/